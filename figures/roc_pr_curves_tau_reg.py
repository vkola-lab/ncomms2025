# Make ROC and PR curves for Tau regional labels
...
def generate_roc(y_true, y_pred, features, figsize=(2.3, 2.3), figname='Average_ROC_curves'):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"{figname}.pdf", format='pdf', dpi=300, bbox_inches='tight')
...
df = pd.read_csv(f'./source_data/stage_2_model_predictions.csv')
# print(df)

y_true_ =  np.array(df[[f'{lab}_label' for lab in labels]])
scores_proba_ =  np.array(df[[f'{lab}_prob' for lab in labels]])

generate_roc(y_true_, scores_proba_, labels, figname=f'./pdf_plots/fig2e')
generate_pr(y_true_, scores_proba_, labels, figname=f'./pdf_plots/fig2f')
